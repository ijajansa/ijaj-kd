...
class EwasteController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('e-collections.all',compact('data'));
    }
    ...
    {
        $data = Recycle::where('type',2)->get();
        return view('e-collections.index',compact('data'));
    }
    ...
    public function create(Request $request)
    {
        ...
        {
            $total += $record->request_e_items_sum_actual_qty;
            $main_total += $record->request_e_items_sum_actual_qty;
        }
        
        $recycle = Recycle::where('type',2)->sum('weight');
        $total -= $recycle;
        
        return view('e-collections.add',compact('total','main_total'));
    }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            return redirect('e-processed')->with($notification);
    }
    ...
    public function exportExcel(Request $request)
    {
        $html='<table border="1">
        <tr>
        <th colspan="14"><h3>VITA MUNICIPAL COUNCIL | E-Waste Processed Report Data</h3></th>
        </tr>
        <tr>
        ...
        $records=Recycle::orderBy('created_at','ASC')->where('type',2)->get();
        ...
        $filename = "e_waste_processed_report_".date('Ymdhis');
