				...
				<div class="card border-top border-0 border-4 border-primary">
					<div class="card-body p-5">
						<div class="card-title d-flex align-items-center">
							...
							<h5 class="mb-0 text-primary" style="font-weight: bold;">Update Category Details</h5>
						</div>
						<hr>
						<form class="row g-3" method="POST" action="{{route('categories.update',$data->id)}}">
							@csrf
							@method('PATCH')
							<div class="col-md-12">
							...
							<div class="col-md-12">
								...
								<select class="form-control form-select @error('type') is-invalid @enderror" name="type">
									<option value="1" @if(old('type',$data->type)==1) selected @endif>Cleaning Monitoring</option>
									<option value="2" @if(old('type',$data->type)==2) selected @endif>Waste Management</option>
								</select>
								@error('type')
                                	<span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
