					...
					<div class="card-body p-5">
						...
						<form class="row g-3" method="POST" action="{{route('products.update',$data->id)}}">
							...
							<div class="col-md-12">
								...
								<input type="text" name="name"  class="form-control @error('name') is-invalid @enderror" value="{{old('name',$data->name)}}" placeholder="Name"  onkeypress="return blockSpecialChar(event)">
								@error('name')
							...
							<div class="col-md-12">
								...
								<select class="form-control form-select @error('type') is-invalid @enderror" name="type">
									...
									<option value="{{$category->id}}" @if(old('type',$data->type)==$category->id) selected @endif>{{$category->name ?? ''}}</option>
									@endforeach
								</select>
								@error('type')
                                	<span class="invalid-feedback" role="alert">
